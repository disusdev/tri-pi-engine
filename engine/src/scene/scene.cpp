...
int addNode(Scene& scene, int parent, int level)
{
	...
	{
		// T-255 resize aux arrays (local/global etc.)
		scene.localTransform_.push_back(mat4_identity());
		scene.globalTransform_.push_back(mat4_identity());
	}
...
void markAsChanged(Scene& scene, int node)
{
	...
	// T-256 use non-recursive iteration with aux stack
...
void loadScene(const char* fileName, Scene& scene)
{
	...
	// T-257 check > -1
	// T-258 recalculate changedAtThisLevel() - find max depth of a node [or save scene.maxLevel]
	fread(scene.localTransform_.data(), sizeof(mat4), sz, f);
	fread(scene.globalTransform_.data(), sizeof(mat4), sz, f);
...
void saveScene(const char* fileName, const Scene& scene)
{
	...
	fwrite(scene.localTransform_.data(), sizeof(mat4), sz, f);
	fwrite(scene.globalTransform_.data(), sizeof(mat4), sz, f);
...
void dumpTransforms(const char* fileName, const Scene& scene)
{
	...
	for (size_t i = 0 ; i < scene.localTransform_.size() ; i++)
	{
		...
		fprintf(f, "Node[%d].globalDet = %f; localDet = %f\n", (int)i, mat4_determinant(scene.globalTransform_[i]), mat4_determinant(scene.localTransform_[i]));
	}
	fclose(f);
}
...
*/
void mergeScenes(Scene& scene, const std::vector<Scene*>& scenes, const std::vector<mat4>& rootTransforms, const std::vector<uint32_t>& meshCounts,
