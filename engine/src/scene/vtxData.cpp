...
MeshFileHeader loadMeshData(const char* meshFile, MeshData& out)
{
	...
	if (fread(out.boxes_.data(), sizeof(bbox), header.meshCount, f) != header.meshCount)
	{
...
void saveMeshData(const char* fileName, const MeshData& m)
{
	...
	fwrite(m.boxes_.data(), sizeof(bbox), header.meshCount, f);
...
void saveBoundingBoxes(const char* fileName, const std::vector<bbox>& boxes)
{
	FILE* f = fopen(fileName, "wb");
	...
	fwrite(boxes.data(), sz, sizeof(bbox), f);

	fclose(f);
}

void loadBoundingBoxes(const char* fileName, std::vector<bbox>& boxes)
{
	...
	// T-259 check file size, divide by bounding box size
	boxes.resize(sz);
	fread(boxes.data(), sz, sizeof(bbox), f);
...
void recalculateBoundingBoxes(MeshData& m)
{
	...
	for (const auto& mesh : m.meshes_)
	{
		...
		for (auto i = 0; i != numIndices; i++)
		{
			...
			vmin = vec3_min(vmin, vec3_create(vf[0], vf[1], vf[2]));
			vmax = vec3_max(vmax, vec3_create(vf[0], vf[1], vf[2]));
		}

		m.boxes_.emplace_back((bbox){ vmin, vmax });
	}
}
