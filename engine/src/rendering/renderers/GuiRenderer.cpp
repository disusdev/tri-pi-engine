...
void GuiRenderer::updateBuffers(size_t currentImage)
{
	...
	const mat4 ormat = mat4_ortho(L, R, T, B);
	// const glm::mat4 inMtx = glm::ortho(L, R, T, B);//  mat4_ortho(L, R, B, T, 0.1f, 1000.0f);
	updateUniformBuffer(currentImage, 0, sizeof(mat4), ormat.data);
	...
	Renderer(ctx)
{
	...
	allTextures.push_back(ctx.resources.createFontTexture("data/comic-code-regular.ttf"));
	for (auto t: textures)
