...
u8* genDefaultCheckerboardImage(int* width, int* height);
...
, envMap_(envMap)
{
	...
	if (asyncLoad)
	{
		...
		std::for_each(std::execution::par, textureFiles_.begin(), textureFiles_.end(),
		[this, idx = 0](std::string& elem) mutable
		{
			int w, h;
			const uint8_t* img = stbi_load(elem.c_str(), &w, &h, nullptr, STBI_rgb_alpha);
			if (!img)
...
void VKSceneData::loadScene(const char* sceneFile)
{
	...
	transforms_ = ctx.resources.addStorageBuffer(shapes_.size() * sizeof(mat4));
